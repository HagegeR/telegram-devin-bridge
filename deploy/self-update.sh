...
# service if anything changed; unprivileged services exit for their supervisor
# to respawn because they cannot invoke the service manager directly.
...
if [ "$(id -u)" -eq 0 ]; then
  ...
  nohup sh -c "sleep 2; kill -TERM $PPID" >/dev/null 2>&1 &
